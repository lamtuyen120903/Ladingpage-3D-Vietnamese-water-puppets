...
const actColors: Record<ActId, { main: string; fill: string; ambient: number }> = {
  intro: { main: '#e080a0', fill: '#4060c0', ambient: 0.7 },
  automation: { main: '#d880a0', fill: '#5070c0', ambient: 0.75 },
  ai: { main: '#e090b0', fill: '#6080d0', ambient: 0.8 },
}
...
export default function StageLighting({ currentAct }: Props) {
  ...
  return (
    <>
      ...
      <ambientLight ref={ambientRef} intensity={0.8} color="#606068" />
      ...
      {/* Backlight blue glow */}
      <pointLight position={[0, 5, -6]} intensity={5} color="#3050a0" distance={18} decay={1.2} />

      {/* Warm gold center + overhead combined */}
      <pointLight position={[0, 4, 1]} intensity={5} color="#d0b060" distance={15} decay={1} />

      {/* Side platform warm fill (both sides, single central light) */}
      <pointLight position={[0, 1.5, 3]} intensity={3.5} color="#e0c880" distance={14} decay={1} />
    </>
  )
}
